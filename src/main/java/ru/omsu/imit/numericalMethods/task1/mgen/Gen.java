...
public class Gen {
    ...
    public void print_matr(double[][] a) {
        System.out.println();
    ...
    public double matr_inf_norm(double[][] a) {
        int i, j;
        double s, norm = 0.;

        for (i = 0; i < a.length; i++) {
            for (s = 0., j = 0; j < a.length; j++) {
                s += Math.abs(a[i][j]);
            }
            if (s > norm) {
                norm = s;
    ...
    public double analyse(double[][] matrix, double[] f, double[] expectedAnswers, double[] answers, String file) {
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(new File(file), true))) {
    ...
    public void mygen(double[][] a, double[][] a_inv, int n, double alpha, double beta, int sign_law, int lambda_law,
                      int variant, int schema, String file) {
        int i, j, k;
                ...
                switch (schema) {
                    case 1:
                        ...
                        for (i = 1; i < n - 1; i++) {
                            ...
                            aa[2] = a[i][i + 1];
                            for (j = 0; j < i; j++) {
                                a[i][j] = aa[0] * (double) (n - i + 1)
                                        + aa[1] * (double) (n - i) + aa[2] * (double) (n - i - 1);
                            }
                            s = aa[0] + aa[1];
        ...
        norm = matr_inf_norm(a);
        System.out.println(" ||  A  || = " + norm);

        norm_inv = matr_inf_norm(a_inv);
